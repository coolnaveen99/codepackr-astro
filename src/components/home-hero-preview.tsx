  ...
  return (
    <div className="relative hidden lg:flex flex-col gap-3">
        ...
        <div className="grid grid-cols-4 overflow-hidden rounded-xl border border-border bg-elevated">
          {GRID.map((sign, i) =>
            sign === null ? (
              ...
              <div key={sign} className="min-h-[58px] border-border border-r border-b bg-surface p-1.5">
                <div className="text-[9px] font-bold uppercase tracking-wide text-muted">{names[sign]}</div>
                {SAMPLE[sign] ? <div className="mt-1 text-[11px] font-bold text-accent">{SAMPLE[sign]}</div> : null}
              </div>
            ),
          )}
        </div>
        <button type="button" onClick={() => go("jathagam")} className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-accent hover:underline">
      ...
      <div className="rounded-2xl border border-accent/20 bg-surface p-4 shadow-card">
        ...
        <button type="button" onClick={() => go("tamil-calendar")} className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-accent hover:underline">
      ...
      <div className="rounded-2xl border border-accent/20 bg-surface px-4 py-3 shadow-card">
        <button type="button" onClick={() => go("porutham")} className="flex w-full items-center justify-between text-left">
          <span className="flex items-center gap-2 text-sm font-bold text-ink">
            <HeartHandshake className="size-4 text-accent" />
            {isTa ? "10 பொருத்தம் — உத்தமம் / மத்தியம் / அதமம்" : "10 Poruthams — Uthamam / Madhimam / Athamam"}
          </span>
